...
class Sun {
    ...
    init() {
        let radiusOfSphere: Float = 0.5
        let numberOfPyramid:Int = 8
        var pyramidNodeArray: [SCNNode] = [SCNNode]()
        let sunColor = hexStringToUIColor(hex: "#FCD440")
        let pyramidWidth: Float = 0.3
        let pyramidHeight: Float = 0.3
        let pyramidLength: Float = 0.3
        ...
        //This position is relative to the camera. now it's 2 meter forward relative to the camera.
        sphereNode.position = SCNVector3(0, 0, 0 - offset)
        self.sphere = sphereNode
        
        let centralOfSphereX = sphereNode.position.x
        let centralOfSphereY = sphereNode.position.y
        let centralOfSphereZ = sphereNode.position.z  + offset
